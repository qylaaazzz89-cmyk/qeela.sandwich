...
<body class="bg-pink-50 font-sans">
  <header class="bg-pink-600 text-white py-4 shadow-md">
    <div class="max-w-6xl mx-auto flex justify-between items-center px-4">
      <h1 class="text-2xl font-bold">🥪 Aqeela Sandwich</h1>
      <button id="cartBtn" class="bg-white text-pink-600 font-semibold px-3 py-1 rounded-md">Keranjang (0)</button>
    </div>
  </header>

  <main class="max-w-6xl mx-auto p-4">
    <div class="flex justify-between mb-4 flex-col sm:flex-row gap-2">
      <input id="search" type="text" placeholder="Cari sandwich..." class="border px-3 py-2 rounded-md w-full sm:w-1/3">
  ...
  <div id="cartPanel" class="fixed inset-y-0 right-0 w-80 bg-white shadow-xl transform translate-x-full transition-transform p-4 overflow-y-auto">
    ...
    <div id="checkout" class="mt-5 border-t pt-3 hidden">
      <h3 class="font-semibold mb-2 text-pink-600">💰 Pembayaran</h3>
      <form id="paymentForm" class="space-y-2">
        ...
        <button type="submit" class="bg-pink-600 text-white font-semibold w-full py-2 rounded-md hover:bg-pink-700">Bayar Sekarang</button>
      </form>
    </div>
  </div>

  <footer class="text-center text-sm text-gray-600 py-4 mt-6">© 2025 Aqeela Sandwich</footer>

  <script>
    const products = [
      {id:'s1', name:'Sandwich Ayam', price:15000, cat:'Ayam', desc:'Roti isi ayam lembut dengan saus spesial buatan Aqeela'},
      {id:'s2', name:'Sandwich Tuna', price:18000, cat:'Tuna', desc:'Sandwich tuna segar dengan sayur dan keju'},
      {id:'s3', name:'Sandwich Telur & Keju', price:12000, cat:'Spesial', desc:'Isian telur orak-arik dan keju lumer lezat'},
      {id:'s4', name:'Sandwich Daging Sapi', price:20000, cat:'Spesial', desc:'Roti panggang isi daging sapi dan saus BBQ'},
      {id:'s5', name:'Sandwich Vegetarian', price:13000, cat:'Spesial', desc:'Sayuran segar dengan saus mayo rendah lemak'}
    ];
    ...
    function renderProducts(list = products) {
      productsDiv.innerHTML = list.map(p => `
        <div class='bg-white rounded-lg shadow p-4 hover:shadow-lg transition'>
          <h3 class='font-bold text-lg mb-1 text-pink-700'>${p.name}</h3>
          <p class='text-sm text-gray-600 mb-2'>${p.desc}</p>
          <div class='flex justify-between items-center'>
            <span class='font-semibold text-pink-600'>Rp ${p.price.toLocaleString()}</span>
            <button onclick='addToCart("${p.id}")' class='bg-pink-600 text-white px-3 py-1 rounded-md hover:bg-pink-700'>Tambah</button>
          </div>
        </div>`).join('');
    }
    ...
    function updateCart(){
      ...
      for(const id in cart){
        ...
        cartItems.innerHTML+=`<div class='flex justify-between items-center border p-2 rounded-md'>
          <div><div class='font-medium text-pink-700'>${p.name}</div><div class='text-sm text-gray-500'>x${qty}</div></div>
